...
const Fashsales = ({ products }) => {
  ...
  return (
    <section className="py-16 mt-10 px-4 md:px-12 lg:px-24">
      ...
      <div className="carousel-overflow-fix" style={{ overflow: "visible" }}>
      <Carousel opts={{ align: "start", loop: false }} className="w-full max-w-6xl mx-auto px-12" style={{ overflow: "visible" }}>
        <CarouselContent className="-ml-6 pl-6" style={{ overflow: "visible" }}>
          {displayProducts.map((item, index) => {
            ...
            return (
              <CarouselItem key={product?.id || index} className="pl-6 md:basis-1/2 lg:basis-1/3 py-4 px-3">
                <div className={[
                  ...
                  <div className="flex flex-col flex-1 p-6 gap-3">
                    <div>
                      <h3 className={[
                        "font-extrabold text-base uppercase tracking-wide transition-colors duration-300",
                        isGoldBest
                      ...
                      <p className="text-white/40 text-[11px] uppercase tracking-wider mt-0.5 line-clamp-2">
                        {product?.description}
                      </p>
                    </div>
                    ...
                    {features.length > 0 && (
                      <ul className="flex flex-col gap-1.5">
                        ...
                        {extraCount > 0 && (
                          <li className="text-cyan-400/60 text-xs mt-1 cursor-pointer hover:text-cyan-400 transition-colors">
                    ...
                    <div className="flex items-center justify-between mt-auto pt-3 border-t border-white/5">
                      ...
                      <Link href={`/products/${product?.id}`}>
                        <button className={[
                          "flex items-center gap-2 px-5 py-2 rounded-lg text-sm font-bold transition-all duration-200 hover:scale-105 active:scale-95",
        ...
        <CarouselPrevious className="absolute -left-20 top-1/2 -translate-y-1/2 w-12 h-12 bg-white/10 border border-white/20 text-white/60 hover:bg-white/20 hover:text-white hover:scale-125 hover:shadow-2xl hover:shadow-white/20 transition-all duration-300 rounded-full p-3 flex items-center justify-center group z-20">
          <ChevronLeft className="w-6 h-6 group-hover:scale-110 transition-transform" />
        </CarouselPrevious>

        <CarouselNext className="absolute -right-20 top-1/2 -translate-y-1/2 w-12 h-12 bg-white/10 border border-white/20 text-white/60 hover:bg-white/20 hover:text-white hover:scale-125 hover:shadow-2xl hover:shadow-white/20 transition-all duration-300 rounded-full p-3 flex items-center justify-center group z-20">
