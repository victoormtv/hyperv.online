...
const Fashsales = ({ products }) => {
  ...
  return (
    <section className="py-16 mt-10 px-4 md:px-12 lg:px-24">
      <div className="text-center mb-10">
        ...
        <div className="mt-5 flex flex-wrap items-center justify-center gap-2">
          {gameTags.map((game) => (
            <span
              key={game}
              className="inline-flex items-center gap-2 rounded-full border border-white/8 bg-white/[0.02] px-3 py-1.5 text-[11px] font-medium text-white/70 transition-colors duration-200 hover:border-cyan-400/30 hover:text-white"
            >
              <Gamepad2 size={13} className="text-cyan-400/80" />
              {game}
            </span>
          ))}
        </div>
      ...
      <div
        className="carousel-overflow-fix relative"
      ...
      <div className="flex justify-center mt-10">
        <Link
          href="/products"
          className="px-8 py-3 rounded-lg border border-white/15 text-white/70 text-sm font-semibold hover:border-cyan-500/40 hover:text-white transition-all duration-200"
        >
          {t.fashsalesViewAll}
        </Link>
      </div>
