      ...
      <div className="flex flex-col flex-1 p-5 gap-3">
        <div>
          <h3 className={[
            ...
            isGoldBest ? "text-white group-hover:text-yellow-300" : "text-white group-hover:text-cyan-400"
          ].join(" ")}>
            {product?.name}
          </h3>
          ...
          <ul className="flex flex-col gap-1.5">
            {visibleFeatures.map((f, i) => (
              <li key={i} className="flex items-center gap-2 text-white/70 text-xs">
                <ShieldCheck size={12} className={isGoldBest ? "text-yellow-400 shrink-0" : "text-cyan-400 shrink-0"} />
                {f}
              </li>
            ))}
            {extraCount > 0 && (
              <li className="text-cyan-400/60 text-xs mt-0.5">
        ...
        <div className="flex items-center justify-between mt-auto pt-3 border-t border-white/5">
          <div>
            <p className="text-white/30 text-[10px] uppercase tracking-wider">{t.fashsalesFrom ?? "From"}</p>
