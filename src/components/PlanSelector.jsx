...
const PlanSelector = ({ plans, product, isGoldBest }) => {
  ...
  const productWithPlan = {
    ...
    planLabel: selectedPlan?.label ?? (t.productPage?.standardPlan ?? "Standard"),
  ...
  return (
    <div className="w-full lg:w-[360px] xl:w-[420px] shrink-0 flex flex-col gap-4 pb-16">
      <div className={["rounded-2xl border border-white/10 bg-[#0d0f14] p-7", cardShadow].join(" ")}>
        <p className="text-white font-extrabold text-2xl mb-6">
          {t.productPage?.choosePlan ?? "Choose Your Plan"}
        </p>
        ...
        <div className="flex flex-col gap-3 mb-6">
          {plans?.length > 0 ? (
            ...
            <div className={["flex items-center justify-between px-5 py-4 rounded-xl border", selectedBorder].join(" ")}>
              <span className="text-white/90 text-lg font-semibold">
                {t.productPage?.standardPlan ?? "Standard"}
              </span>
              <span className={["font-extrabold text-lg", accent].join(" ")}>
                {isOutOfStock ? (t.fashsalesOutOfStock ?? "Out of stock") : `$${product?.price}`}
              </span>
            </div>
          )}
        </div>
        ...
        {isOutOfStock ? (
          <div className="w-full flex items-center justify-center gap-2 py-4 rounded-xl font-extrabold text-base bg-red-500/15 text-red-400 border border-red-500/30 cursor-not-allowed">
            {t.fashsalesOutOfStock ?? "Out of stock"}
          </div>
        ) : (
          <AddToCart product={productWithPlan}>
            <div
              className={[
                "w-full flex items-center justify-center gap-2 py-4 rounded-xl font-extrabold text-base cursor-pointer transition-all duration-200 hover:scale-105 active:scale-95",
                buyBtn,
              ].join(" ")}
            >
              {isFree ? (
                <>
                  ...
                  {t.checkout?.getFree ?? "Get for Free"} →
                </>
              ) : (
                <>
